...
export default function Header() {
  ...
  return (
    <header className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-full flex justify-center pointer-events-none">
      <div
        className={`
          pointer-events-auto w-full max-w-[90rem]
            ...
            return (
              <a
                ...
                className={`
                  ...
                  ${
                    isActive
                      ? "text-black font-semibold bg-[#e6daae] shadow-[0_0_18px_rgba(230,218,174,0.55)] border border-yellow-200/60"
                      : "text-[#e6daae] hover:text-white hover:bg-white/10 rounded-full"
                  }
                `}
              >
                {link.label}
                ...
                {/* GOLD GLOW OUTLINE */}
                {isActive && (
                  <span className="absolute inset-0 rounded-full border border-[#fff6d5]/30 shadow-[0_0_12px_4px_rgba(230,218,174,0.25)]"></span>
                )}
              </a>
            );
          })}
      ...
      {isOpen && (
        <div
          className="
            absolute top-full mt-4 w-[85%] left-1/2 -translate-x-1/2
            bg-[#0a1624]/90 rounded-2xl border border-white/10 
            backdrop-blur-xl shadow-2xl p-6 space-y-4 
            md:hidden text-[#e6daae] font-inter
          "
        >
          {navLinks.map((link) => {
            const isActive = pathname === link.href;
